...
const CONFIG = {
    ...
    ROOT: document.getElementById('vtfilms-app')
};
...
const nav = (view, value = '', slug = '', ep = '', page = 1) => {
    ...
    if (view === 'watch')       url += `?watch=${value}${ep ? '&ep=' + ep : ''}`;
        ...
        // slug param reused as searchType when view=search (phim/dien-vien/dao-dien/tu-khoa)
        const stype = (slug && slug !== '') ? slug : 'phim';
        url += `?search=${encodeURIComponent(value)}&stype=${stype}&page=${page}`;
    }
...
<div id="vtf-page-loader" class="VTFilms_PageLoader">
    <div class="VTFilms_PageLoader-spinner">
        ...
        <span class="VTFilms_PageLoader-text" style="color:var(--vtf-primary)">VT Films</span>
    </div>
</div>`;
        ...
        <div class="VTFilms_Menu-left">
            ...
            <div class="VTFilms_Menu-search">
                ...
                <input type="text" class="VTFilms_Menu-search-input"
                       placeholder="Tìm kiếm phim, từ khóa..."
...
/* ─────────────────────────────────────────────
   PAGINATION
───────────────────────────────────────────── */
function paginationHtml(type, slug, keyword, totalPages, current, stype = '') {
    if (totalPages <= 1) return '';
    const link = (p) => keyword
        ? `javascript:nav('search','${keyword}','${stype}','',${p})`
...
/* ─────────────────────────────────────────────
   GRID PAGE
───────────────────────────────────────────── */
async function renderGrid(type, slug, keyword, searchType = 'phim') {
    document.title = 'VT Films';
    CONFIG.ROOT.innerHTML = pageLoaderHtml();
    ...
    // ── Tìm kiếm ─────────────────────────────────────────────────────────────
    // [v5.3.0] Chiến lược 3 tab dựa trên API thực tế:
    //
    //  API endpoints được dùng:
    //  - /tim-kiem?keyword=X&page=N  → danh sách phim (slug, name, thumb_url, ...)
    //                                   KHÔNG có actor/director/keywords
    //  - /phim/[slug]/peoples        → { data: { peoples: [{name,...}], keywords: [{name, name_vn,...}] } }
    //
    //  Tab "phim"     → /tim-kiem, hiện tất cả kết quả (OPhim đã search server-side)
    //  Tab "dien-vien"→ /tim-kiem page 1 → lấy slugs
    //                   → fetch /phim/[slug]/peoples song song
    //                   → filter những phim có peoples[].name chứa keyword
    //  Tab "tu-khoa"  → /tim-kiem page 1 → lấy slugs
    //                   → fetch /phim/[slug]/peoples song song
    //                   → filter những phim có keywords[].name hoặc .name_vn chứa keyword

    const norm = (s) => (s || '').normalize('NFC').toLowerCase();
    const kw   = norm(keyword);

    const TABS = [
        { key: 'phim',      label: 'Phim',      icon: 'fa-film' },
        { key: 'dien-vien', label: 'Diễn viên', icon: 'fa-user-group' },
        { key: 'tu-khoa',   label: 'Từ khóa',   icon: 'fa-magnifying-glass' },
    ];
    ...
    if (searchType === 'phim') {
        // Tab Phim: /tim-kiem trả về kết quả là phim, hiển thị tất cả
        const data = await fetchJson(`${CONFIG.API}/tim-kiem?keyword=${encodeURIComponent(keyword)}&page=${page}`);
        displayItems = data?.data?.items || [];
        const pag  = data?.data?.params?.pagination;
        total      = pag?.totalItems || 0;
        totalPages = Math.ceil(total / (pag?.totalItemsPerPage || 24));
        metaText   = `${total.toLocaleString('vi-VN')} kết quả &bull; Trang ${page}/${totalPages}`;

    } else if (searchType === 'dien-vien' || searchType === 'tu-khoa') {
        // Tab Diễn viên / Từ khóa:
        // Bước 1: lấy danh sách phim từ /tim-kiem (trang 1, tối đa 24 phim)
        const searchData  = await fetchJson(`${CONFIG.API}/tim-kiem?keyword=${encodeURIComponent(keyword)}&page=1`);
        const listItems   = searchData?.data?.items || [];

        // Bước 2: fetch /phim/[slug]/peoples song song cho tất cả slugs
        // /peoples trả về cả peoples[] lẫn keywords[] trong 1 lần gọi
        const peoplesResponses = await Promise.allSettled(
            listItems.map(item => fetchJson(`${CONFIG.API}/phim/${item.slug}/peoples`))
        );

        // Bước 3: ghép kết quả peoples/keywords vào từng item, rồi filter
        const enriched = listItems.map((item, idx) => {
            const res = peoplesResponses[idx];
            const d   = (res.status === 'fulfilled' && res.value?.data) ? res.value.data : {};
            return {
                ...item,
                _peoples:  d.peoples  || [],   // [{name, character, ...}]
                _keywords: d.keywords || [],    // [{name, name_vn, ...}]
            };
        });

        if (searchType === 'dien-vien') {
            // Lọc phim có bất kỳ diễn viên nào (peoples[].name) chứa keyword
            displayItems = enriched.filter(m =>
                m._peoples.some(p => norm(p.name).includes(kw))
            );
            metaText = `${displayItems.length} phim có diễn viên "<strong>${keyword}</strong>"`;
        } else {
            // Lọc phim có từ khóa (keywords[].name hoặc .name_vn) chứa keyword
            displayItems = enriched.filter(m =>
                m._keywords.some(k => norm(k.name).includes(kw) || norm(k.name_vn).includes(kw))
            );
            metaText = `${displayItems.length} phim có từ khóa "<strong>${keyword}</strong>"`;
        }
    }

    baseTitle = keyword ? `Tìm kiếm: "${keyword}"` : 'Tìm kiếm';
    document.title = `${baseTitle} — VT Films`;

    const kw_escaped  = keyword.replace(/'/g, "\\'");
    const searchTabsHtml = `
    <div class="VTFilms_Search-tabs">
        ${TABS.map(t => `
        <button class="VTFilms_Search-tab${searchType === t.key ? ' VTFilms_Search-tab--active' : ''}"
                onclick="nav('search','${kw_escaped}','${t.key}')">
            <i class="fad ${t.icon}"></i> ${t.label}
        </button>`).join('')}
    </div>`;
    ...
    <div class="VTFilms_Search-empty">
        ...
        Không tìm thấy <strong>"${keyword}"</strong> trong mục <strong>${TABS.find(t => t.key === searchType)?.label || 'này'}</strong>.
    </div>` : '';
    ...
    <div class="VTFilms_Grids">
        ...
        ${searchType === 'phim' && totalPages > 1
            ? paginationHtml('search', '', keyword, totalPages, page, searchType)
            : ''}
    </div>` + footerHtml();
}
...
function appRouter() {
    ...
    if (p.get('search')) return renderGrid('search', '', p.get('search'), p.get('stype') || 'phim');
...
/* ─────────────────────────────────────────────
   AUTH-GATED ROUTER BOOTSTRAP
   - __VTF_READY = true  (cache says valid)   → run appRouter immediately on DOMContentLoaded
   - __VTF_READY = false (no cache / pending) → wait for vtfilms:auth-ready event from module
   - popstate: only route if user is verified (no SPA navigation for blocked users)
───────────────────────────────────────────── */
window.onpopstate = function() {
    if (window.__VTF_READY) appRouter();
};

document.addEventListener('DOMContentLoaded', function() {
    if (window.__VTF_READY) {
        // Fast path: cache confirms valid user — show app + run router immediately
        const appEl = document.getElementById('vtfilms-app');
        if (appEl) appEl.classList.remove('d-none');
        appRouter();
    } else {
        // Slow path: wait for Firebase module to confirm auth state
        window.addEventListener('vtfilms:auth-ready', function(e) {
            if (e.detail && e.detail.user
                && window.VTFilms_Auth && window.VTFilms_Auth.isVerified()) {
                appRouter();
            }
        }, { once: true });
    }
