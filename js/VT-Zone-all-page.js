...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 2. Hàm Helper: Tạo attribute cho Fancybox
    const getFancyboxAttrs = (item) => {
        // Cố định data-type="image" nếu không phải video để Fancybox không bị lỗi nhận diện
        if (item.type !== 'video') {
    ...
    function renderCarousel() {
        ...
        let html = '';
        // Sử dụng index để tạo Unique ID, tránh lỗi trùng lặp do ký tự đặc biệt
        Object.keys(groupedData).forEach((title, index) => {
            const groupItems = groupedData[title];
            const mainItem = groupItems[0];
            
            // SỬA LỖI Ở ĐÂY: Dùng index thay vì regex title
            const groupId = `group-story-${index}`; 
