...
function VT_homePostLayout() {
    ...
    postContainers.forEach((container) => {
        ...
        if (count > 0) {
            ...
            gallery.className = `VT_homePostGallery p-0 m-0 mt-3 layout-${displayCount}`;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 2. Hàm Helper: Tạo attribute cho Fancybox (để Template bắt lấy)
    const getFancyboxAttrs = (item) => {
        if (item.type === 'video' && item.ratio) {
            let attrs = `data-aspect-ratio="${item.ratio}"`;
            
            // Giữ lại các thông số kích thước để video 9/16 hiển thị đẹp trên mobile
            if (item.ratio === '9/16') {
                attrs += ` data-width="1406" data-height="2500"`;
            } else if (item.ratio === '16/9') {
                attrs += ` data-width="1920" data-height="1080"`;
            }
            return attrs;
        }
        return '';
    ...
    function renderCarousel() {
        ...
        Object.keys(groupedData).forEach(title => {
            ...
            // Tạo ID nhóm an toàn hơn (loại bỏ ký tự đặc biệt)
    ...
    function updateButtons() {
        ...
        // Cho phép sai số nhỏ (2px) do browser zoom/scaling
    ...
    // Khởi chạy
    renderCarousel();
    // Đợi 1 chút để hình ảnh load layout rồi mới tính toán nút lần đầu cho chuẩn
